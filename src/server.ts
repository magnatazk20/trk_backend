...
app.post('/api/vip/activate', requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    try {
      ...
      // Desconta SOMENTE da carteira selecionada pelo usuário
      const walletBalance = Number(users[0][walletField] ?? 0)
      ...
      if (walletField === 'balance') {
        ...
        balanceAfter = walletBalance - levelPrice
      ...
      const [purchaseResult] = await conn.query<ResultSetHeader>(
        ...
        VALUES (?, ?, ?, ?, ?)
        `,
        [parsedUserId, parsedVipLevelId, levelPrice, walletBalance, balanceAfter]
      )
...
const applyReferralCommissionsForDeposit = async (cashinPaymentId: number, depositorUserId: number, depositAmount: number) => {
  ...
  try {
    ...
    for (let level = 1; level <= 3; level += 1) {
      ...
      const parentVipLevelId = Number(t0CheckRows[0]?.vipLevelId ?? 0)
      if (parentVipLevelId === 6) {
        // T0-Estágio: não recebe comissão de depósito, mas o loop continua subindo na árvore
        currentUserId = parentUserId
        continue
      }
