...
const normalizePixType = (pixTypeRaw) => {
...
const mapPixTypeToLumopay = (pixTypeRaw) => {
...
const normalizeLumopayPixKey = (pixKeyRaw, lumopayPixType) => {
...
const normalizePixKey = (pixKeyRaw, pixType) => {
    const key = String(pixKeyRaw ?? '').trim();
    switch (pixType) {
        case 'CPF':
        case 'CNPJ':
...
const resolveAuthUser = async (req) => {
    ...
    try {
        ...
        if (!userId || Number.isNaN(userId))
            return null;
        const [rows] = await db_1.default.query('SELECT id, phone, is_admin AS isAdmin FROM users WHERE id = ? LIMIT 1', [userId]);
        if (rows.length === 0)
            return null;
        return {
            id: Number(rows[0].id),
...
const processTelegramUpdates = async () => {
    try {
      ...
      `);
        ...
        const configuredCheckinSuccessMessage = String(configRows[0].checkinSuccessMessage ?? '').trim() ||
            'Check-in do dia {day} resgatado com sucesso!';
        const configuredCheckinAlreadyClaimedMessage = String(configRows[0].checkinAlreadyClaimedMessage ?? '').trim() ||
        ...
        for (const update of updatesData.result) {
          ...
          `, [telegramUserId]);
                ...
                const checkinDay = Number(claimResult?.claim?.day ?? 0);
                const usernameValue = telegramUsername ? `@${telegramUsername}` : '';
                const displayNameValue = usernameValue ||
                    String(telegramFirstName ?? '').trim() ||
                    'usuário';
                const interpolateTelegramTemplate = (templateRaw) => String(templateRaw ?? '')
                ...
                const alreadyClaimed = !claimResult.ok &&
                    String(claimResult?.error ?? '').trim().toLowerCase() === 'check-in de hoje já foi resgatado.';
...
const requireMaxAdmin = async (req, res, next) => {
    const authUser = await resolveAuthUser(req);
    if (!authUser) {
        res.status(401).json({ ok: false, error: 'Não autorizado.' });
        return;
...
const settleExpiredCyclesForUser = async (userId) => {
    if (!userId || Number.isNaN(userId))
        return;
    const conn = await db_1.default.getConnection();
    try {
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db_1.default.query('SELECT id FROM users WHERE phone = ?', [phone]);
        if (rows.length > 0) {
        ...
        if (referralCode) {
            const [refRows] = await db_1.default.query('SELECT id FROM users WHERE referral_code = ? LIMIT 1', [referralCode]);
            if (refRows.length > 0) {
...
app.post('/api/auth/login', authLimiter, async (req, res) => {
    ...
    try {
        // Buscar usuário
        const [rows] = await db_1.default.query('SELECT id, name, phone, password, is_admin FROM users WHERE phone = ?', [phone]);
        if (rows.length === 0) {
        ...
        // Gerar JWT
        const token = jsonwebtoken_1.default.sign({ id: user.id, phone: user.phone }, JWT_SECRET, { expiresIn: JWT_EXPIRES });
        res.json({
...
app.post('/api/CASHIN/', async (req, res) => {
    ...
    try {
        ...
        const [users] = await db_1.default.query('SELECT id, name, phone FROM users WHERE id = ?', [parsedUserId]);
        if (users.length === 0) {
            res.status(404).json({ error: 'Usuário não encontrado.' });
            return;
        }
        ...
        const lumoData = await lumoResponse.json();
        ...
        const transactionId = lumoData?.data?.id ??
            lumoData?.data?.transaction_id ??
            lumoData?.data?.payment_data?.transaction_id ??
            null;
        const qrCode = lumoData?.data?.payment_data?.qr_code ??
            lumoData?.data?.payment_data?.pix_code ??
            '';
        const qrImage = lumoData?.data?.payment_data?.qr_code_image ??
            lumoData?.data?.payment_data?.qr_code_base64 ??
            '';
        const providerStatus = String(lumoData?.data?.status ??
            lumoData?.status ??
            'pending');
        const [paymentResult] = await db_1.default.query(`
...
app.post('/api/CASHIN/webhook', express_1.default.raw({ type: '*/*' }), async (req, res) => {
    try {
        ...
        const calculated = crypto_1.default
            ...
            .digest('hex');
        if (!signature || !crypto_1.default.timingSafeEqual(Buffer.from(calculated), Buffer.from(String(signature)))) {
        ...
        if (providerTransactionId) {
            const [existingRows] = await db_1.default.query('SELECT id, status, user_id, amount FROM cashin_payments WHERE provider_transaction_id = ? LIMIT 1', [String(providerTransactionId)]);
        ...
        `, [
          ...
          `, [providerTransactionId ? String(providerTransactionId) : '']);
                if (createdRows.length > 0) {
                    await applyReferralCommissionsForDeposit(Number(createdRows[0].id), userId, amountBRL);
                }
            }
        }
        res.status(200).send('OK');
...
app.get('/api/referral/:userId', async (req, res) => {
    ...
    try {
        const [rows] = await db_1.default.query('SELECT id, referral_code AS referralCode FROM users WHERE id = ? LIMIT 1', [userId]);
        if (rows.length === 0) {
            res.status(404).json({ ok: false, error: 'Usuário não encontrado.' });
            return;
        }
        ...
        if (!referralCode) {
            ...
            await db_1.default.query('UPDATE users SET referral_code = ? WHERE id = ?', [referralCode, userId]);
        }
...
app.get('/api/vip/user/:userId', async (req, res) => {
    ...
    try {
        const [balanceRows] = await db_1.default.query('SELECT balance FROM users WHERE id = ? LIMIT 1', [userId]);
...
app.post('/api/vip/activate', async (req, res) => {
    const { userId, vipLevelId } = req.body;
    const parsedUserId = Number(userId);
    ...
    try {
        await settleExpiredCyclesForUser(parsedUserId);
        const [users] = await db_1.default.query('SELECT id, balance FROM users WHERE id = ? LIMIT 1', [parsedUserId]);
        if (users.length === 0) {
            res.status(404).json({ ok: false, error: 'Usuário não encontrado.' });
            return;
        }
        const [levels] = await db_1.default.query('SELECT id, name, price, is_active AS isActive FROM vip_levels WHERE id = ? LIMIT 1', [parsedVipLevelId]);
...
app.get('/api/mining/tasks/:userId', async (req, res) => {
    ...
    try {
        ...
        const [vipRows] = await db_1.default.query(`
      SELECT
        uv.id,
        uv.vip_level_id AS vipLevelId,
        vl.name AS vipName,
...
app.post('/api/mining/tasks/complete', async (req, res) => {
    ...
    try {
      ...
      `, [parsedUserId, saoPauloDate]);
        ...
        const [taskRows] = await db_1.default.query(`
      ...
      `, [parsedTaskId]);
        ...
        const [progressRows] = await db_1.default.query(`
...
app.get('/api/profile/metrics/:userId', async (req, res) => {
    ...
    try {
        await settleExpiredCyclesForUser(userId);
        const [users] = await db_1.default.query('SELECT id, balance FROM users WHERE id = ? LIMIT 1', [userId]);
        if (users.length === 0) {
            res.status(404).json({ ok: false, error: 'Usuário não encontrado.' });
            return;
        }
        const withdrawableBalance = Number(users[0].balance ?? 0);
        const [teamRows] = await db_1.default.query('SELECT COUNT(*) AS total FROM users WHERE referred_by_user_id = ?', [userId]);
      ...
      `);
        const [cycleRows] = await db_1.default.query(`
      SELECT
        ucp.id,
...
app.post('/api/cycle-products/purchase', async (req, res) => {
    const { userId, cycleProductId } = req.body;
    const parsedUserId = Number(userId);
...
app.get('/api/team/report/:userId', async (req, res) => {
    ...
    try {
        ...
        const [kpiRows] = await db_1.default.query(`
      SELECT
        (SELECT COUNT(*)
         FROM users u
         WHERE u.referred_by_user_id = ?
      ...
      `, hasDateRange
        ...
        let teamWithdraw = 0;
        try {
            const [withdrawRows] = await db_1.default.query(`
        ...
        const [levelRows] = await db_1.default.query(`
      WITH RECURSIVE referral_tree AS (
        SELECT
          u.id,
...
app.post('/api/community-links', async (req, res) => {
    const { whatsappGroupUrl, vipGroupUrl, managerContact } = req.body;
    ...
    try {
      ...
      `);
        const [rows] = await db_1.default.query('SELECT id FROM community_links ORDER BY id ASC LIMIT 1');
        if (rows.length === 0) {
            await db_1.default.query(`
...
app.get('/api/transactions/paid/:userId', async (req, res) => {
    ...
    try {
        ...
        const [depositRows] = await db_1.default.query(`
      SELECT
        id,
        amount,
        method,
    ...
    try {
      ...
      `, [userId]);
        const normalizeStatus = (statusRaw) => {
            ...
            if (value === 'paid' || value === 'payment.paid')
                return 'paid';
            if (value === 'processing')
                return 'processing';
...
app.get('/api/checkin/status/:userId', async (req, res) => {
    ...
    try {
      ...
      `);
        ...
        const [todayRows] = await db_1.default.query(`
      ...
      LIMIT 10
      `, [userId]);
        const [latestRows] = await db_1.default.query(`
      SELECT checkin_day AS checkinDay
      FROM daily_checkins
      WHERE user_id = ?
      ORDER BY id DESC
...
app.post('/api/user/pix-key', async (req, res) => {
    const { userId, holderName, holderCpf, pixKeyType, pixKey } = req.body;
    const parsedUserId = Number(userId);
    if (!parsedUserId || Number.isNaN(parsedUserId)) {
        res.status(400).json({ ok: false, error: 'ID de usuário inválido.' });
        return;
...
app.post('/api/user/withdraw-password', async (req, res) => {
    ...
    try {
        const [users] = await db_1.default.query('SELECT id, password FROM users WHERE id = ? LIMIT 1', [parsedUserId]);
        if (users.length === 0) {
            res.status(404).json({ ok: false, error: 'Usuário não encontrado.' });
            return;
        }
        const isSameAsLoginPassword = await bcrypt_1.default.compare(parsedPassword, String(users[0].password ?? ''));
        if (isSameAsLoginPassword) {
            res.status(400).json({ ok: false, error: 'A senha de saque não pode ser igual à senha de login.' });
            return;
        }
        const passwordHash = await bcrypt_1.default.hash(parsedPassword, 10);
        await db_1.default.query(`
      UPDATE users
...
app.post('/api/user/change-password', async (req, res) => {
    ...
    try {
        ...
        const validCurrentPassword = await bcrypt_1.default.compare(parsedCurrentPassword, currentHash);
        if (!validCurrentPassword) {
            res.status(401).json({ ok: false, error: 'Senha atual incorreta.' });
            return;
        }
        const isSamePassword = await bcrypt_1.default.compare(parsedNewPassword, currentHash);
        if (isSamePassword) {
            res.status(400).json({ ok: false, error: 'A nova senha não pode ser igual à senha atual.' });
            return;
        }
        const newPasswordHash = await bcrypt_1.default.hash(parsedNewPassword, 10);
        await db_1.default.query(`
      UPDATE users
      SET password = ?
      WHERE id = ?
...
app.post('/api/withdraw/request', async (req, res) => {
    ...
    try {
      ...
      `, [parsedUserId]);
        ...
        const validPassword = await bcrypt_1.default.compare(parsedPassword, String(users[0].withdrawPassword ?? ''));
        if (!validPassword) {
            await conn.rollback();
...
app.post('/api/withdraw/webhook', async (req, res) => {
    ...
    try {
      ...
      `);
        ...
        const normalizedStatus = statusUpper === 'COMPLETED' || statusUpper === 'PAID' || statusUpper === 'PAYMENT.PAID'
            ? 'paid'
        ...
        const providerTransactionId = String(payload?.providerTransactionId ??
        ...
        if (providerTransactionId) {
            const [rowsByProviderId] = await db_1.default.query(`
        ...
        if (!foundWithdrawal && externalId) {
            const [rowsByExternalId] = await db_1.default.query(`
        SELECT id, user_id AS userId, amount, status, pix_key AS pixKey
        FROM withdrawals
        WHERE external_id = ?
        ORDER BY id DESC
        ...
        if (!foundWithdrawal && pixKey) {
            const [rowsByPix] = await db_1.default.query(`
        SELECT id, user_id AS userId, amount, status, pix_key AS pixKey
        FROM withdrawals
        WHERE pix_key = ?
        ...
        if (!foundWithdrawal && amount != null) {
            const [rowsByAmount] = await db_1.default.query(`
        SELECT id, user_id AS userId, amount, status, pix_key AS pixKey
        FROM withdrawals
        ...
        if (!foundWithdrawal) {
            const [rowsMostRecent] = await db_1.default.query(`
        SELECT id, user_id AS userId, amount, status, pix_key AS pixKey
        FROM withdrawals
        ORDER BY created_at DESC
        LIMIT 1
        ...
        `, [
            ...
            const shouldRefund = normalizedStatus === 'failed' &&
...
const applyReferralCommissionsForDeposit = async (cashinPaymentId, depositorUserId, depositAmount) => {
    ...
    if (!parsedPaymentId || Number.isNaN(parsedPaymentId))
        return;
    if (!parsedDepositorUserId || Number.isNaN(parsedDepositorUserId))
        return;
    ...
    try {
      ...
      `);
        ...
        `, [currentUserId]);
            ...
            if (!parentUserId || Number.isNaN(parentUserId))
                break;
            ...
            if (levelConfig) {
                ...
                if (commissionAmount > 0) {
                    const [existingPayoutRows] = await conn.query(`
            SELECT id
...
const ensureGiftCodeTables = async () => {
    ...
    const tryAlter = async (sql) => {
        try {
            ...
            const ignorable = code === 'ER_DUP_FIELDNAME' ||
...
app.delete('/api/admin/gift-codes/:id', requireMaxAdmin, async (req, res) => {
    ...
    try {
        await ensureGiftCodeTables();
        await conn.beginTransaction();
        const [existsRows] = await conn.query('SELECT id, code FROM gift_codes WHERE id = ? LIMIT 1 FOR UPDATE', [giftCodeId]);
        if (existsRows.length === 0) {
            await conn.rollback();
            res.status(404).json({ ok: false, error: 'Código não encontrado.' });
            return;
        }
        await conn.query('DELETE FROM gift_code_redemptions WHERE gift_code_id = ?', [giftCodeId]);
        await conn.query('DELETE FROM gift_codes WHERE id = ?', [giftCodeId]);
        await conn.commit();
        res.json({
            ok: true,
...
app.post('/api/gift-codes/redeem', async (req, res) => {
    ...
    try {
      ...
      `, [normalizedCode]);
        ...
        const [alreadyRedeemedRows] = await conn.query(`
      SELECT id
      ...
      `, [Number(gift.id)]);
        ...
        const [updatedUserRows] = await conn.query('SELECT balance FROM users WHERE id = ? LIMIT 1', [parsedUserId]);
        ...
        res.json({
            ...
            config: {
                ...
                alreadyLinkedMessage: String(rows[0].alreadyLinkedMessage ?? '').trim() ||
                    'Esta conta já foi conectada anteriormente e não pode ser vinculada novamente.',
                checkinSuccessMessage: String(rows[0].checkinSuccessMessage ?? '').trim() || 'Check-in do dia {day} resgatado com sucesso!',
                checkinAlreadyClaimedMessage: String(rows[0].checkinAlreadyClaimedMessage ?? '').trim() || 'Check-in de hoje já foi resgatado.',
                updatedAt: rows[0].updatedAt ?? null,
            },
        });
    }
...
app.post('/api/admin/telegram-config', requireMaxAdmin, async (req, res) => {
    ...
    const parsedAlreadyLinkedMessage = String(alreadyLinkedMessage ?? '').trim() ||
        'Esta conta já foi conectada anteriormente e não pode ser vinculada novamente.';
    const parsedCheckinSuccessMessage = String(checkinSuccessMessage ?? '').trim() ||
        'Check-in do dia {day} resgatado com sucesso!';
    const parsedCheckinAlreadyClaimedMessage = String(checkinAlreadyClaimedMessage ?? '').trim() ||
        'Check-in de hoje já foi resgatado.';
    if (!parsedBotToken) {
...
app.post('/api/admin/site-settings', requireMaxAdmin, async (req, res) => {
    const { siteTitle, siteDescription, siteLogoUrl } = req.body;
    ...
    try {
      ...
      `);
        ...
        const [rows] = await db_1.default.query('SELECT id FROM site_settings ORDER BY id ASC LIMIT 1');
        if (rows.length === 0) {
            await db_1.default.query(`
        INSERT INTO site_settings (site_title, site_description, site_logo_url)
        VALUES (?, ?, ?)
...
app.post('/api/admin/withdrawals/:id/action', requireMaxAdmin, async (req, res) => {
    const withdrawalId = Number(req.params.id);
    const { action, refundOnCancel, provider } = req.body;
    ...
    try {
      ...
      `, [withdrawalId]);
        ...
        let providerTransactionId = null;
        let providerResponsePayload = null;
        if (parsedAction === 'approve') {
            const [pixRows] = await conn.query(`
...
app.post('/api/admin/migrate-balance-columns', async (_req, res) => {
    try {
        const [balanceCols] = await db_1.default.query("SHOW COLUMNS FROM users LIKE 'balance'");
        const [depositCols] = await db_1.default.query("SHOW COLUMNS FROM users LIKE 'total_deposits'");
        if (balanceCols.length === 0) {
            await db_1.default.query('ALTER TABLE users ADD COLUMN balance DECIMAL(12,2) NOT NULL DEFAULT 0.00');
        }
        if (depositCols.length === 0) {
            await db_1.default.query('ALTER TABLE users ADD COLUMN total_deposits DECIMAL(12,2) NOT NULL DEFAULT 0.00');
        }
        await db_1.default.query(`
...
app.get('/api/admin/rankings', requireMaxAdmin, async (_req, res) => {
    try {
        const [referralRows] = await db_1.default.query(`
      WITH RECURSIVE referral_tree AS (
        SELECT
          u.id AS rootUserId,
...
app.patch('/api/admin/users/:id/ban', requireMaxAdmin, async (req, res) => {
    const userId = Number(req.params.id);
    const isBanned = Number(req.body?.is_banned ?? 0) ? 1 : 0;
    if (!userId || Number.isNaN(userId)) {
        res.status(400).json({ ok: false, error: 'ID inválido.' });
        return;
    }
...
app.post('/api/admin/users/:id/balance', requireMaxAdmin, async (req, res) => {
    const userId = Number(req.params.id);
    const { amount, operation, reason } = (req.body ?? {});
    const parsedAmount = Number(String(amount ?? '').replace(',', '.'));
    const parsedOperation = String(operation ?? '').trim();
    ...
    try {
      ...
      `);
        const [rows] = await conn.query('SELECT id, balance FROM users WHERE id = ? LIMIT 1 FOR UPDATE', [userId]);
        if (rows.length === 0) {
            await conn.rollback();
            res.status(404).json({ ok: false, error: 'Usuário não encontrado.' });
            return;
        ...
        const nextBalance = parsedOperation === 'add'
...
app.get('/api/admin/logs', requireMaxAdmin, async (req, res) => {
    ...
    try {
      ...
      LIMIT ?
      `, [limit]);
        const resolveCategory = (actionRaw, entityTypeRaw) => {
            const action = String(actionRaw ?? '').toLowerCase();
            const entityType = String(entityTypeRaw ?? '').toLowerCase();
            if (action.includes('withdraw') || entityType === 'withdrawal')
                return 'withdraw';
            if (action.includes('cashin') || action.includes('deposit') || entityType === 'cashin')
                return 'deposit';
            if (action.includes('balance') || action.includes('admin_balance'))
                return 'balance';
            if (action.includes('password') || action.includes('login') || action.includes('security'))
                return 'security';
            if (action.includes('gift') || action.includes('code'))
                return 'gift_code';
...
app.get('/api/admin/users/:id/details', requireMaxAdmin, async (req, res) => {
    ...
    try {
      ...
      `, [userId]);
        ...
        let accountLogs = [];
        try {
            const [logRows] = await db_1.default.query(`
        SELECT
          id,
          action,
          old_balance,
        ...
        let cyclePurchases = [];
        try {
            const [cycleRowsDetailed] = await db_1.default.query(`
        SELECT
          ucp.id,
        ...
        let giftCodeRedemptions = [];
        try {
            const [giftRows] = await db_1.default.query(`
        SELECT
          gcr.id,
        ...
        let dailyCheckinRedemptions = [];
        try {
            const [checkinRows] = await db_1.default.query(`
        SELECT
          id,
